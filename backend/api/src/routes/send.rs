...
pub async fn send_email(
    ...
    let account: Option<(String, String, Vec<u8>, String, i64, String)> = sqlx::query_as(
        "SELECT id, primary_email, credentials_encrypted, smtp_host, smtp_port, smtp_auth_scheme FROM email_accounts WHERE id = ?",
    )
    .bind(&req.account_id)
    .fetch_optional(&user_db)
    .await?;

    let (account_id, primary_email, creds_enc, smtp_host, smtp_port, smtp_auth_scheme) =
    ...
    // Build and send via smtp crate
    let (message_id, raw_sent) = smtp::send(smtp::SendRequest {
        from: req.from.clone(),
        to: req.to.clone(),
    ...
    if let Some(host) = imap_host {
        ...
        let _ = imap_sync::append_to_sent(
            &host,
            imap_port as u16,
            &imap_user,
            &imap_pass,
