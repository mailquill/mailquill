...
fn mail_list(c: &mut Criterion) {
    ...
    group.bench_function("unified_inbox_page50", |b| {
        b.iter(|| {
            rt.block_on(async {
                db::queries::unified_page(&db, Some("inbox"), None, 50)
                    .await
                    .unwrap()
            })
        })
    ...
    group.bench_function("unified_inbox_page50_cursor", |b| {
        ...
        let cursor = rt.block_on(async {
            let p = db::queries::unified_page(&db, Some("inbox"), None, 50)
                .await
                .unwrap();
            p.next_cursor
        });
        b.iter(|| {
            rt.block_on(async {
                db::queries::unified_page(&db, Some("inbox"), cursor.as_deref(), 50)
                    .await
                    .unwrap()
            })
        })
